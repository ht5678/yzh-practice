...
public class TrieTest
{
    ...
    {
        BufferedReader br = new BufferedReader(new InputStreamReader(getClass().getResourceAsStream("/resources/dictionary.txt")));
        String line;
    ...
    @Test
    public void nonOverlappingCn()
    {
        Trie trie = new Trie().removeOverlaps();
        trie.addKeyword("中国");
        trie.addKeyword("水瓶");
        trie.addKeyword("环境污染");
        Collection<Emit> emits = trie.parseText("中国的环境污染一半都是水.瓶,垃圾造成的");
        assertEquals(2, emits.size());
        Iterator<Emit> iterator = emits.iterator();
        // With overlaps: ab@1, ab@3, ababc@4, cba@6, ab@7
        checkEmit(iterator.next(), 0, 4, "ababc");
