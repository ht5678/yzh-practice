 ...
 *
 */
public class ThreadScopeData2 {

	//因为每个线程对象都是唯一数据的这个关系
	private static ThreadLocal<Integer> buyThreadForDataMap = new ThreadLocal<>();
	...
	static class A{
		//拿每个线程带来的数据进行处理
		public int get(){
			int data = buyThreadForDataMap.get();
			//业务处理
